	...
	for area in get_overlapping_areas():
		...
		if area.is_in_group("Wall_Bodies"):
			...
			if wall.scale.x > wall.scale.y:
				...
				if (player.position.y + 10 > wall.position.y and bullet.position.y < wall.position.y) or ((player.position.y + 10 < wall.position.y and bullet.position.y > wall.position.y)):
					return;
			else:# Vertical wall
				# If bullet and player are on different sides, ignore it
